...
def run(data):
    ...
    all_chunks_dict = [
        {"start": df["start"].iloc[j], "end": df["end"].iloc[j], "bbox": list(map(int, df["bbox"].iloc[j].split("-"))), "frames": []}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--video_folder", default="youtube-voxceleb", help="Path to youtube videos")
    parser.add_argument("--metadata", default="vox-metadata.csv", help="Path to metadata")
    parser.add_argument("--out_folder", default="vox", help="Path to output")
